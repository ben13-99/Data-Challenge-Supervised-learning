# E14 — E12 (features métier + clipping/log) avec balayage C étendu.
# Phase A: sélection (use_total_cost, use_prev_ratio, clip) avec C=1.0
# Phase B: balayage C ∈ {0.25, 0.5, 1, 2, 4, 8} avec la meilleure config de A
...
def _build_pipeline(use_total_cost: bool, use_prev_ratio: bool,
    ...
    clf = LinearSVC(
        ...
        tol=1e-3,
        max_iter=20000,          # un peu plus haut si C grand
...
def load(train_csv_path: str) -> None:
    """
    Phase A: sélection (use_total_cost, use_prev_ratio, clip) avec C=1.0
    Phase B: balayage C étendu avec la config de A
    ...
    # ---- Phase A : flags + clip avec C=1.0 ----
    USE_TOTAL = [False, True]
    USE_RATIO = [False, True]
    CLIPS = [(1.0, 99.0), (2.0, 98.0)]
    SKEW = [1.0]

    bestA = {"ut": None, "ur": None, "clip": None, "f1": -np.inf}
    for ut in USE_TOTAL:
        for ur in USE_RATIO:
            for (pl, ph) in CLIPS:
                for sk in SKEW:
                    pipe = _build_pipeline(
                        ...
                        dual=dual_flag_tr, C=1.0, random_state=RS
                    )
                    pipe.fit(X_tr, y_tr)
                    y_hat = pipe.predict(X_va)
                    f1w = f1_score(y_va, y_hat, average="weighted")
                    if f1w > bestA["f1"]:
                        bestA.update({"ut": ut, "ur": ur, "clip": (pl, ph), "f1": f1w})

    # ---- Phase B : balayage C étendu ----
    C_GRID = [0.25, 0.5, 1.0, 2.0, 4.0, 8.0]
    bestB = {"C": None, "f1": -np.inf}
    for C in C_GRID:
        pipe = _build_pipeline(
    ...
    _pipe = _build_pipeline(
        use_total_cost=bestA["ut"], use_prev_ratio=bestA["ur"],
        p_low=bestA["clip"][0], p_high=bestA["clip"][1], skew_thresh=1.0,
        dual=dual_flag_all, C=bestB["C"], random_state=RS
    )
    _pipe.fit(X, y)
    # print(f"[E14] Best: ut={bestA['ut']}, ur={bestA['ur']}, clip={bestA['clip']}, C={bestB['C']} (valid F1_w={bestB['f1']:.4f}).")
